...
async def send_code(
    ...
    # AuthRestartError — Telegram tomonidan chiqadigan, odatda vaqtinchalik
    # xato (server IP/DC almashinuvi va h.k. bilan bog'liq). Telethon'ning
    # o'zi buni odatda ichkarida hal qiladi, lekin ba'zan chiqib qoladi —
    # shu holatda ODDIY QAYTA URINISH yordam beradi (2 marta, qisqa kutish bilan).
    sent = None
    last_error = None
    ...
    if sent is None:
        ...
        raise HTTPException(
            500,
            f"Telegram vaqtinchalik xato berdi (AuthRestartError). "
...
async def verify_code(
    ...
    # MUHIM: sessiya ENDI MATN sifatida (client.session.save()) Neon
    # bazasiga yoziladi — Render diskiga bog'liq emas, deploy bo'lsa
    # ham yo'qolmaydi.
    await access_service.connect_access_account(
        db, owner_id=user.id, phone=payload.phone,
        session_file=client.session.save(), pool=get_pool(),
    )
