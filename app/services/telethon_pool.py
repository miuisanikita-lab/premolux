...
class TelethonPool:
    ...
    async def premium_enter_card(self, client: TelegramClient, invoice_msg,
        ...
        """
        Telegram to'lov formasini so'raydi.

        DIQQAT: SmartGlocal'ning to'g'ridan-to'g'ri API'sini (tokenize
        endpoint) qo'lda chaqirish SINALDI, lekin doim "No public token"
        xatosi berdi — chunki bu API ularning O'Z JavaScript vidjeti
        orqali maxsus protokol (sessiya/imzo) bilan chaqirilishi kerak,
        buni tashqaridan taxmin qilib takrorlab bo'lmaydi.

        Shuning uchun ISHONCHLIROQ yo'l tanlandi: SmartGlocal'ning
        HAQIQIY, ISHLAYDIGAN veb-sahifasiga (form.url) o'tkazamiz —
        bu yerda ularning o'z JavaScript kodi TO'G'RI ishlaydi, chunki
        bu ularning haqiqiy, mijozlar ishlatadigan sahifasi. Karta
        ma'lumotini kiritish/OTP kutish bank_page.py da amalga oshadi.
        """
        ...
        if not pay_url:
            raise RuntimeError("To'lov sahifasi manzili (url) topilmadi")
