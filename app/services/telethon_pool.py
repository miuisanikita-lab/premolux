...
async def _test_proxy(conf: tuple, timeout: float = 6.0) -> bool:
    """Proxy orqali Telegram serveriga tez ulanish sinovi."""
    import asyncio
    try:
        import socks
        sock = socks.socksocket()
        sock.set_proxy(socks.SOCKS5, conf[1], conf[2])
        sock.settimeout(timeout)
        loop = asyncio.get_event_loop()
        await asyncio.wait_for(
            loop.run_in_executor(None, sock.connect, ("149.154.167.50", 443)),
            timeout=timeout,
        )
        sock.close()
        return True
...
async def default_proxy_conf():
    ...
    qaytaradi (natijani keshlab, qayta-qayta sinamaydi). Ro'yxat
    bo'sh bo'lsa — eski, bitta-proxy sozlamasiga tushadi.
    """
    ...
    for conf in candidates:
        if conf in _tried_dead:
            continue
        ok = await _test_proxy(conf)
        if ok:
            _working_proxy_cache = conf
            return conf
        _tried_dead.add(conf)

    # ro'yxat tugadi yoki bo'sh — eski bitta-proxy sozlamasiga qaraymiz
