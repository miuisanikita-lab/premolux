...
class TelethonPool:
    ...
    async def premium_enter_card(self, client: TelegramClient, invoice_msg,
        ...
        from telethon.tl.types import InputInvoiceMessage, InputPaymentCredentials
        ...
        # MUHIM DIAGNOSTIKA: to'lov provayderi (masalan SmartGlocal)
        # kartani QANDAY formatda kutishini bilish uchun, formaning
        # haqiqiy tuzilishini logga chiqaramiz — bu birinchi sinovda
        # ANIQ formatni topish uchun zarur (taxmin bilan yozib bo'lmaydi,
        # chunki ko'p provayderlar kartani shifrlab yuborishni talab qiladi)
        print(f"[PAYMENT FORM] provider={getattr(form, 'native_provider', None)}")
        print(f"[PAYMENT FORM] native_params={getattr(form, 'native_params', None)}")
        print(f"[PAYMENT FORM] url={getattr(form, 'url', None)}")
        print(f"[PAYMENT FORM] form_id={getattr(form, 'form_id', None)}")
        print(f"[PAYMENT FORM] barcha maydonlar={form.to_dict() if hasattr(form, 'to_dict') else form}")

        pay_url = getattr(form, "url", None)

        # Agar "native" provayder bo'lsa (masalan SmartGlocal) — buni
        # ko'rsatib, HOZIRCHA to'g'ridan-to'g'ri url'ga tayanamiz.
        # native_params ichida ko'pincha shifrlash kaliti (public_key)
        # bo'ladi — buni ko'rgach, ANIQ shifrlash usulini qo'shamiz.
        native_provider = getattr(form, "native_provider", None)
        if native_provider and not pay_url:
            raise RuntimeError(
                f"Native to'lov provayderi topildi ({native_provider}) — "
                f"karta shifrlash formati aniqlashtirilishi kerak. "
                f"Loglardagi [PAYMENT FORM] qatorlarini tekshiring."
            )
