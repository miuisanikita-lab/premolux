...
class TelethonPool:
    ...
    async def click_button(self, message, label_contains: str) -> str:
        """
        Xabardagi tugmani matn bo'yicha topib bosadi, keyin xabarni
        QAYTA YUKLAB, tahrirlangan (yangilangan) matnni o'qiydi.

        MUHIM: Telethon Message obyektida .get_message() metodi
        MAVJUD EMAS — bu avvalgi versiyada doim ESKI matnni
        qaytarishga olib kelgan edi (Code/Pass hech qachon
        topilmagan bo'lardi). Endi client.get_messages() orqali
        bot tahrirlagan xabar HAQIQATAN qayta o'qiladi.
        """
        if message is None:
        ...
        for row in (message.buttons or []):
            for btn in row:
                if label_contains.lower() in (btn.text or "").lower():
                    ...
                    # bot xabarni tahrirlashi uchun kutamiz — VAQT
                    # HAR XIL bo'lishi mumkin, shuning uchun BIR MARTA
                    # emas, bir necha marta tekshiramiz (polling),
                    # matn ESKISIDAN farq qilishi bilanoq to'xtaymiz
                    client = message.client
                    original_text = getattr(message, "raw_text", "")
                    text = original_text
                    for _ in range(6):  # jami ~6 soniyagacha
                        await asyncio.sleep(1.0)
                        fresh = await client.get_messages(message.chat_id, ids=message.id)
                        fresh_text = getattr(fresh, "raw_text", None) or ""
                        if fresh_text and fresh_text != original_text:
                            text = fresh_text
                            break
                        text = fresh_text or text
                    return text
