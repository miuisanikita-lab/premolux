...
class TelethonPool:
    ...
    async def premium_enter_card(self, client: TelegramClient, invoice_msg,
        ...
        """
        Telegram to'lov formasini so'raydi, karta ma'lumotini SmartGlocal'ning
        TOKENIZE API'siga to'g'ridan-to'g'ri yuboradi (Playwright/veb-sahifa
        EMAS), so'ng olingan tokenni Telegram'ga SendPaymentFormRequest
        orqali yuboradi.

        MANBA: Telegram'ning o'zining rasmiy iOS ilovasi kodi
        (BotCheckoutNativeCardEntryControllerNode.swift) — bu yerda aynan
        shu oqim ko'rsatilgan: tokenize URL'ga so'rov, natijada tokenId
        olinadi, keyin {"type":"card","id":"<tokenId>"} JSON SendPaymentForm
        ga uzatiladi.
        """
        ...
        print(f"[PAYMENT FORM] provider={native_provider} currency={getattr(getattr(form, 'invoice', None), 'currency', None)} amount={getattr(getattr(form, 'invoice', None), 'prices', None)}")
        ...
        # ── 1-QADAM: karta ma'lumotini SmartGlocal API'siga TO'G'RIDAN-
        # TO'G'RI (Telegram serverlariga emas) yuboramiz, tokenlanadi ──
        async with httpx.AsyncClient(timeout=20) as http:
            resp = await http.post(
                ...
                json={
                    "type": "card",
                    "public_token": public_token,   # sarlavha yetarli bo'lmadi — tana ichida ham yuboramiz
                    "card": {
                        "number": number,
                        "expiration_month": exp_month.strip(),
                        "expiration_year": exp_year.strip()[-2:],  # SmartGlocal aniq 2 xonali yil talab qiladi
                        "security_code": cvv,
                    },
                },
            )
        ...
        token_id = (
            token_data.get("token", {}).get("token")
            if isinstance(token_data.get("token"), dict)
            else token_data.get("token") or token_data.get("id")
        )
        if not token_id:
            raise RuntimeError(f"SmartGlocal token qaytmadi: {token_data}")

        # ── 2-QADAM: olingan tokenni Telegram'ga yuboramiz ──
        credentials_data = _json.dumps({"type": "card", "id": token_id})
        ...
        # muvaffaqiyat bo'lsa — payments.PaymentResult yoki
        # payments.PaymentVerificationNeeded (3-D Secure kerak) qaytadi
        verify_url = getattr(send_result, "url", None)
        if verify_url:
            return {"payUrl": verify_url, "formId": form.form_id, "native": True}

        # 3-D Secure kerak bo'lmadi — to'lov DARROV tugagan bo'lishi mumkin
