...
async def _fill_card_fields(page: Page, number: str, exp: str, cvv: str) -> "Page | None":
    """
    ...
    MUHIM: karta yuborilgach (Done/Enter bosilgach), TO'LOV
    ko'pincha YANGI OYNADA (popup/yangi tab) ochiladi — ESKI page
    buni HECH QACHON ko'rmaydi! Shuning uchun bu funksiya endi
    yangi oyna ochilishini KUTIB, agar ochilsa O'SHA YANGI Page
    obyektini QAYTARADI (chaqiruvchi shu yangisiga o'tishi kerak).
    Agar yangi oyna ochilmasa — None qaytaradi (eski page davom etadi).
    """
    ...
    # MUHIM: BOSISHDAN OLDIN — yangi oyna (popup) ochilishini
    # "tinglashni" boshlaymiz. Ko'p to'lov tizimlari to'lov tugmasi
    # bosilganda YANGI TAB/OYNA ochadi (window.open() orqali) —
    # bu holatda ESKI page hech qachon haqiqiy to'lov/OTP sahifasini
    # ko'rmaydi, chunki u BOSHQA oynada.
    context = page.context
    new_page_future = asyncio.ensure_future(context.wait_for_event("page", timeout=8000))
    ...
    # Yangi oyna ochilganmi — tekshiramiz (8s gacha kutamiz)
    new_page = None
    try:
        new_page = await new_page_future
        await new_page.wait_for_load_state("domcontentloaded", timeout=10000)
        print(f"[CARD FILL] YANGI OYNA OCHILDI: {new_page.url}")
    except Exception:
        if not new_page_future.done():
            new_page_future.cancel()
        print("[CARD FILL] Yangi oyna ochilmadi (kutilgan holat bo'lishi mumkin)")

    if new_page:
        # YANGI oynada davom etamiz — bu haqiqiy to'lov/OTP sahifasi
        return new_page
    ...
    #
    # MUHIM TUZATISH: karta maydonlari IFRAME ichida ekanligi
    # diagnostikada ko'ringan edi — shuning uchun HAQIQIY to'lov
    # tugmasi ham iframe ICHIDA bo'lishi mumkin. Faqat asosiy
    # sahifada emas, BARCHA frame'larda (asosiy + iframe'lar) qidiramiz.
    try:
        pay_hint_btn = None
        pay_text = "?"
        max_wait_s = 30   # yetarli vaqt — dinamik tekshiruv bo'lgani uchun tezroq topilsa ertaroq chiqadi
        check_every_s = 0.5
        elapsed = 0.0

        # yana yangi oyna kutish (2-tugma bosilganda ham ochilishi mumkin)
        new_page_future2 = asyncio.ensure_future(context.wait_for_event("page", timeout=int(max_wait_s * 1000)))

        while elapsed < max_wait_s:
            for frame in [page] + page.frames:
                for hint in SUBMIT_TEXT_HINTS:
                    try:
                        btn = frame.get_by_role("button", name=re.compile(hint, re.I)).first
        ...
        if pay_hint_btn:
            ...
            try:
                new_page2 = await asyncio.wait_for(
                    asyncio.shield(new_page_future2), timeout=8
                )
                await new_page2.wait_for_load_state("domcontentloaded", timeout=10000)
                print(f"[CARD FILL] 2-QADAMDAN KEYIN YANGI OYNA: {new_page2.url}")
                return new_page2
            except Exception:
                pass
        else:
    ...
    return page   # yangi oyna ochilmadi — ESKI page bilan davom etamiz
...
async def open_page_and_wait_otp(url: str, number: str = "", exp: str = "", cvv: str = "",
                                  timeout_ms: int = 25000, cookies: list | None = None):
    """
    ...
    async with _browser_semaphore:
        return await _open_page_and_wait_otp_inner(url, number, exp, cvv, timeout_ms, cookies)


async def _open_page_and_wait_otp_inner(url: str, number: str, exp: str, cvv: str,
                                         timeout_ms: int, cookies: list | None = None):
    """
    ...
    browser = None
    try:
        # MUHIM: ILGARI bu yerda PROXY UMUMAN ISHLATILMAGAN edi —
        # bank sahifasiga har doim SERVERNING O'ZINING haqiqiy IP
        # manzili orqali kirilardi. Ko'p marta ketma-ket urinish bir
        # xil IP'dan kelsa, bank/Checkout.com buni "shubhali" deb
        # bloklashi mumkin edi. Endi proxy ro'yxatidan TASODIFIY
        # biri ishlatiladi (har chaqiruvda BOSHQA IP).
        chromium_proxy = None
        try:
            from app.services.telethon_pool import default_proxy_conf
            proxy_conf = await default_proxy_conf()
            if proxy_conf:
                # proxy_conf = (kind, host, port, rdns, user, pass)
                _, phost, pport, _, puser, ppass = proxy_conf
                chromium_proxy = {"server": f"socks5://{phost}:{pport}"}
                if puser and ppass:
                    chromium_proxy["username"] = puser
                    chromium_proxy["password"] = ppass
                print(f"[PROXY] Chromium uchun proxy: {phost}:{pport}")
        except Exception as e:
            print(f"[PROXY] Proxy olishda xato, proxysiz davom etamiz: {e}")
        ...
        if chromium_proxy:
            launch_kwargs["proxy"] = chromium_proxy

        browser = await pw.chromium.launch(**launch_kwargs)
        page = await browser.new_page()

        # MUHIM: agar SmartGlocal karta tokenlash (httpx) bosqichida
        # cookie o'rnatilgan bo'lsa — ularni SHU YERDA, sahifaga
        # o'tishdan OLDIN, brauzer kontekstiga qo'shamiz. httpx va
        # Playwright — ALOHIDA "brauzerlar" bo'lgani uchun, cookie
        # avtomatik almashmaydi; buni QO'LDA bog'lab qo'yamiz.
        if cookies:
            try:
                await page.context.add_cookies(cookies)
                print(f"[COOKIES] {len(cookies)} ta cookie brauzerga qo'shildi")
            except Exception as e:
                print(f"[COOKIES] Cookie qo'shishda xato: {e}")
        ...
        # MUHIM: "sessions-interceptor" / "device-information" —
        # bu Checkout.com'ning QURILMA TEKSHIRUVI (fingerprinting)
        # sahifasi, HAQIQIY OTP sahifasi EMAS. Bu sahifa JavaScript
        # orqali (URL o'zgarishisiz, SPA uslubida) haqiqiy bank
        # sahifasiga o'tishi mumkin — shuning uchun URL "barqaror"
        # ko'ringan bo'lsa ham, YANA BIR NECHA MARTA tekshirib,
        # shu maxsus so'zlar YO'QOLGUNCHA (yoki maksimal 20s)
        # qo'shimcha kutamiz.
        FINGERPRINT_MARKERS = ["sessions-interceptor", "device-information"]
        if any(m in page.url for m in FINGERPRINT_MARKERS):
            print(f"[FINGERPRINT] Qurilma tekshiruv sahifasida: {page.url} — kutilmoqda...")
            fp_deadline_s = 20
            fp_elapsed = 0.0
            while fp_elapsed < fp_deadline_s:
                await page.wait_for_timeout(1000)
                fp_elapsed += 1.0
                if not any(m in page.url for m in FINGERPRINT_MARKERS):
                    print(f"[FINGERPRINT] Sahifadan chiqdi ({fp_elapsed:.0f}s): {page.url}")
                    break
            else:
                print(f"[FINGERPRINT] {fp_deadline_s}s dan keyin ham hali shu "
                      f"sahifada: {page.url} — baribir davom etamiz")
        ...
        if number and exp and cvv:
            try:
                result_page = await _fill_card_fields(page, number, exp, cvv)
                if result_page is not None and result_page is not page:
                    # MUHIM: karta yuborilgach YANGI OYNA ochilgan —
                    # endi SHU YANGI oyna bilan davom etamiz, eski
                    # oynani emas
                    print(f"[CARD FILL] Yangi oynaga o'tildi: {result_page.url}")
                    page = result_page
                elif result_page is page:
...
async def _find_otp_field(page: Page, timeout_ms: int):
    ...
    while asyncio.get_event_loop().time() < deadline:
        # MUHIM XAVFSIZLIK: agar sahifa HALI Checkout.com'ning qurilma
        # tekshiruv (fingerprinting) bosqichida bo'lsa — "yagona input"
        # zaxira mantig'i (2-usul) ISHLATILMAYDI, chunki bu sahifada
        # tasodifiy topilgan maydon HAQIQIY OTP maydoni BO'LMASLIGI
        # extremely mumkin (aniqlik uchun faqat 1-usul — aniq so'z
        # bilan bog'liq maydon — qabul qilinadi).
        on_fingerprint_page = any(m in page.url for m in ["sessions-interceptor", "device-information"])
        ...
        for frame in _all_frames():
            ...
            if on_fingerprint_page:
                continue  # zaxira mantiqqa O'TMAYMIZ — noto'g'ri maydon topib qo'yishi mumkin
