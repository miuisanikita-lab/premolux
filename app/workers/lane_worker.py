...
# Telegram sessiyani O'ZI bekor qilganda chiqadigan xato turlari —
# bularni ko'rsak, hisobni QAYTA ULASH kerakligi ANIQ, qayta urinish
# FOYDASIZ (chunki sessiyaning o'zi endi yaroqsiz).
SESSION_DEAD_ERRORS = (AuthKeyUnregisteredError, SessionRevokedError,
                        UserDeactivatedError, UserDeactivatedBanError)


async def _mark_access_account_disconnected(db: AsyncSession, lane: Lane) -> None:
    """
    MUHIM: Telegram sessiyani BEKOR QILGANINI (masalan xavfsizlik
    tizimi tomonidan chiqarib yuborilgan) aniqlasak, dostup hisobni
    DARHOL "uzilgan" deb belgilaymiz — aks holda tizim jimgina
    xato berib turaveradi, foydalanuvchi buni bilmay qoladi.
    """
    try:
        ...
        access = (await db.execute(
            select(AccessAccount).where(
                AccessAccount.owner_id == order.owner_id,
                AccessAccount.connected == True,  # noqa: E712
            )
        )).scalars().first()
        if access:
            access.connected = False
            await db.commit()
            print(f"[Lane {lane.id}] Dostup hisob (id={access.id}) 'uzilgan' deb belgilandi — "
                  f"sessiya Telegram tomonidan bekor qilingan")
    except Exception as e:
        print(f"[Lane {lane.id}] Dostup hisobni belgilashda xato: {e}")
    ...
    async with SessionLocal() as db:
        ...
        for attempt in range(1, MAX_LOGIN_ATTEMPTS + 1):
            try:
                ...
                print(f"[Lane {lane_id}] SESSIYA BEKOR QILINGAN (login bosqichida): {e}")
                await _mark_access_account_disconnected(db, lane)
                await _fail(db, lane, f"Dostup hisob sessiyasi bekor qilingan — qaytadan ulang")
                return
            except Exception as e:
                import traceback
        ...
        try:
            ...
            print(f"[Lane {lane_id}] SESSIYA BEKOR QILINGAN (@PremiumBot bosqichida): {e}")
            await _mark_access_account_disconnected(db, lane)
            await _fail(db, lane, f"Dostup hisob sessiyasi bekor qilingan — qaytadan ulang")
...
async def _step_buy_premium(db: AsyncSession, lane: Lane, pool: TelethonPool) -> None:
    ...
    try:
        ...
        form = await asyncio.wait_for(
            ...
            timeout=60,   # ichida 15s qo'shimcha kutish qo'shilgani uchun oshirildi
        )
    except asyncio.TimeoutError:
    ...
    saved_cookies = form.get("cookies", [])
    ...
    #
    # MUHIM: bu chaqiruv ham CHEKSIZ osilib qolishi mumkin edi
    # (brauzer, tarmoq va h.k.) — qattiq vaqt chegarasi qo'shildi. ──
    try:
        if is_native:
            pw, browser, page = await asyncio.wait_for(
                bank_page.open_page_and_wait_otp(pay_url, cookies=saved_cookies), timeout=100,
            )
        else:
            pw, browser, page = await asyncio.wait_for(
                bank_page.open_page_and_wait_otp(pay_url, number=number, exp=card.exp, cvv=cvv),
                timeout=100,
            )
    except asyncio.TimeoutError:
        raise LaneError("Bank sahifasi 60s ichida javob bermadi (vaqt tugadi)")
