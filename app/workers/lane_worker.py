...
from app.models.tables import Lane, LaneStatus, Card, NumberBot, AccessAccount, Proxy
...
async def _step_request_code(db: AsyncSession, lane: Lane, pool: TelethonPool) -> None:
    ...
    try:
        updated_text = await asyncio.wait_for(
            pool.click_button(bot_msg, "get code"),
...
async def _step_full_login(db: AsyncSession, lane: Lane, pool: TelethonPool) -> None:
    proxy = await db.get(Proxy, lane.proxy_id) if lane.proxy_id else None
...
async def _step_buy_premium(db: AsyncSession, lane: Lane, pool: TelethonPool) -> None:
    ...
    invoice_msg = await pool.premium_flow_start(client, months=1)
    form = await pool.premium_enter_card(client, invoice_msg, number=number, exp=card.exp, cvv=cvv)

    pay_url = form.get("payUrl")
    if not pay_url:
        raise LaneError("To'lov sahifasi manzili olinmadi")

    # bank OTP so'rasa — shu yerda kutamiz, forwarder orqali keladi
    ...
    # OTP Telegram ichida emas, BANK sahifasida kiritiladi (Playwright)
    ok = await bank_page.submit_otp(pay_url, otp, bank_id=card.bank_id)
    if not ok:
...
async def _step_check_premium(db: AsyncSession, lane: Lane, pool: TelethonPool) -> bool:
    ...
    result_text = await pool.click_button(bot_msg, "check premium")
...
async def _get_access_account(db: AsyncSession, lane: Lane) -> AccessAccount:
    ...
    """
    order = await db.get(lane.order.__class__, lane.order_id)
